...
class TestJSON(unittest.TestCase):
    def test_encoder(self):
        ...
        eq_(json.dumps(data, cls=CustomJSONEncoder), json.dumps(result))
    ...
    def test_decoder(self):
        ...
        eq_(json.loads(data, cls=CustomJSONDecoder), {'x': 1, 'y': date})
        eq_(
            json.loads(data, cls=CustomJSONDecoder, object_pairs_hook=AttrDict),
            AttrDict([('x', 1), ('y', date)]),
        )
    ...
    def test_roundtrip(self):
        ...
        self.check_roundtrip(
            AttrDict([('str', 'x'), ('bool', True), ('num', 1), ('date', tz_date)])
        )
