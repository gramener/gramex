...
async function render(q) {
  ...
  if (q.cookie) {
    ...
    for (let key in cookieObj) {
      ...
      if (q.domain) cookie.domain = q.domain;
  ...
  if (q.emulate) {
    ...
    device.viewport.deviceScaleFactor =
      +q.scale || device.viewport.deviceScaleFactor;
  ...
  if (q.ext == "pdf") {
    // TODO: header / footer
    if (q.media != "print") await page.emulateMediaType(q.media);
    ...
    if (q.ext == "pptx") {
      ...
      repeat_cols.forEach((col) => {
        ...
        for (let i = q[col].length; i < max_length; i++) q[col].push(last_val);
      });
      const image_files = [];
      let pages = _.zip(
        q.selector,
