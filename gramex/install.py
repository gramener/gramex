...
def run_command(config):
    ...
    if cygcheck is not None and cygpath is not None:
        ...
        app_path = check_output(        # nosec 404
            [cygpath, '-au', which(appcmd[0])], **kwargs).strip()
        is_cygwin_app = check_output(   # nosec 404
            [cygcheck, '-f', app_path], **kwargs).strip()
        if is_cygwin_app:
            target = check_output(      # nosec 404
                [cygpath, '-au', target], **kwargs).strip()
