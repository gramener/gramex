...
def _table_status(engine, tables):
    ...
    if _STATUS_METHODS.get(key, None) is None:
        ...
        if dialect == 'mysql':
            ...
            q = (
                'SELECT update_time FROM information_schema.tables WHERE ' +   # nosec B608
                _wheres('table_schema', 'table_name', db, tables))
        elif dialect == 'snowflake':
            # https://docs.snowflake.com/en/sql-reference/info-schema/tables.html
            # B608:hardcoded_sql_expressions only used internally
            q = (
                'SELECT last_altered FROM information_schema.tables WHERE ' +  # nosec B608
                _wheres('table_schema', 'table_name', db, tables))
        elif dialect == 'mssql':
            # https://goo.gl/b4aL9m
            # B608:hardcoded_sql_expressions only used internally
            q = (
                'SELECT last_user_update FROM sys.dm_db_index_usage_stats WHERE ' +  # nosec B608
