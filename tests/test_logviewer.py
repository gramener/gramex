...
class TestLogViewer(TestGramex):
    ...
    def test_endpoints(self):
        ...
        for top in topten:
            cond = df[top['col'] + '_1'].eq(1) if top.get('flag') else slice(None)
            eq_(
                ...
                (
                    ...
                    .astype(int)
                    .rename_axis(top['col'])
                    .reset_index(name=top['values'])
                    .sort_values(by=[top['values'], top['col']], ascending=[False, True])[:10]
                    .to_dict('r')
                ),
            )
        # check trend
        dff = logviewer.pdagg(df[df_uri1], [{'key': 'time', 'freq': 'D'}], {'duration': ['count']})
    ...
    def test_pdagg(self):
        ...
        afe(logviewer.pdagg(self.df, [{'key': 'time', 'freq': 'D'}], {'duration': ['count']}), dfe)

    def test_prepare_where(self):
        eq_(
            logviewer.prepare_where('SELECT * FROM tb', {'col1>': ['10']}, ['col1']),
            'WHERE "col1" > "10"',
        )
        eq_(
            logviewer.prepare_where(
                'SELECT * FROM tb WHERE col2 > 1', {'col1>': ['10']}, ['col1']
            ),
            'AND "col1" > "10"',
        )
        eq_(logviewer.prepare_where('', {'col1>': ['10']}, ['col1']), 'WHERE "col1" > "10"')
