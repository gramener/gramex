...
class TestLDAPAuth(TestGramex):
    ...
    def test_ldap_search(self):
        r = self.check(
            'euler', 'password', url='/auth/ldap2-search', status_code=OK, redirect='/auth/session'
        )
        result = r.json()
    ...
    def test_ldap_bind(self):
        self.check('gauss@ldap.forumsys.com', 'password', url='/auth/ldap2-bind', status_code=OK)

    def test_ldap_bind_wrong_bind(self):
        self.check(
            'anything',
            'password',
            url='/auth/ldap2-bind-wrong-bind',
            status_code=UNAUTHORIZED,
            headers={'Auth-Error': 'bind'},
        )

    def test_ldap_bind_wrong_search(self):
        self.check(
            'nonexistent@ldap.forumsys.com',
            'password',
            url='/auth/ldap2-bind',
            status_code=UNAUTHORIZED,
            headers={'Auth-Error': 'search'},
        )
